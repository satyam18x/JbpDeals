...
const OfferCard = ({ offer, business }) => {
  ...
  const isSaved = savedOffers.some(id => String(id) === offerId);
  ...
  const handleSave = (e) => {
    ...
    e.stopPropagation(); // VERY IMPORTANT: Stop the Link click from being triggered!
    if (user) {
      toggleSaveOffer(offerId).then(res => {
        if (!res.success) {
          console.error("Bookmark Error:", res.message);
        }
      });
    } else {
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-2 mb-3 mt-auto">
          ...
          <span className="text-sm font-medium text-text-secondary truncate">
            {offer.businessName || business?.name || 'Local Business'}
          </span>
        </div>
