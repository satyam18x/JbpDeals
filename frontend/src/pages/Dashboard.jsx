...
const Dashboard = () => {
  ...
  const initialTab = searchParams.get('tab') === 'redeemed' ? 'redeemed' : 'saved';
  const [activeTab, setActiveTab] = useState(initialTab); // 'saved' or 'redeemed'
  const [allOffers, setAllOffers] = useState([]);
  const [loading, setLoading] = useState(true);

  // Sync tab state when searchParams change (from URL navigation)
  useEffect(() => {
    const tab = searchParams.get('tab');
    if (tab === 'saved' || tab === 'redeemed') {
      setActiveTab(tab);
    }
  }, [searchParams]);

  // Update URL searchParams when tab state changes
  ...
  const savedOfferItems = allOffers.filter(o => {
    const oId = String(o._id || o.id);
    return savedOffers.some(sId => String(sId) === oId);
  });

  // Get full offer objects for redeemed items
  const redeemedOfferItems = redeemedCodes
    .map(item => {
      const offer = allOffers.find(o => (o._id || o.id) === item.offerId);
        ...
        <div className="w-full md:w-80 flex-shrink-0">
          <div className="bg-card rounded-3xl p-2 md:p-6 border border-border-color shadow-sm md:sticky md:top-24">
            ...
            <div className="hidden md:flex flex-col items-center text-center pb-6 border-b border-border-color relative">
              ...
              <img src={user.avatar || `https://api.dicebear.com/7.x/initials/svg?seed=${user.name}`} alt={user.name} className="w-24 h-24 rounded-full border-4 border-background shadow-md mb-4" />
              ...
              {savedOfferItems.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {savedOfferItems.map(offer => {
                    ...
                    return <OfferCard key={offer._id || offer.id} offer={offer} business={business} />;
                  })}
                </div>
              ) : (
                      ...
                      <div key={idx} className="bg-card border border-border-color rounded-2xl p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-6 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="flex-grow text-center sm:text-left">
                          ...
                          <div className="flex items-center justify-center sm:justify-start gap-2 text-sm text-text-secondary mb-3">
                            <span className="font-bold">{offer.businessName || 'Local Business'}</span>
                          </div>
