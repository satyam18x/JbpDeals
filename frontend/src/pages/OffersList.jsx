...
import { MOCK_BUSINESSES, MOCK_CATEGORIES } from '../data/mockData';
...
const OffersList = () => {
  ...
  useEffect(() => {
    ...
    if (activeCategory !== 'all') {
      result = result.filter(offer => offer.category === activeCategory || offer.categoryId === activeCategory);
    }
    ...
    if (searchQuery.trim() !== '') {
      ...
      result = result.filter(offer => {
        ...
        const matchBusiness = offer.businessName ? offer.businessName.toLowerCase().includes(lowerQuery) 
                           : (MOCK_BUSINESSES.find(b => b.id === offer.businessId)?.name.toLowerCase().includes(lowerQuery) || false);
        return matchTitle || matchBusiness;
      });
    }

    setFilteredOffers(result);
  }, [searchQuery, activeCategory, allOffers]);
