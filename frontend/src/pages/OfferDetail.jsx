...
const OfferDetail = () => {
    ...
    const fetchOffer = async () => {
      try {
        ...
        if (data && (data._id || data.id)) {
          setOffer(data);
          // Business info is currently embedded as businessName in the Deal model
          setBusiness({ name: data.businessName || 'Local Business', logo: null });
          
          // Check if already redeemed
          const existing = redeemedCodes.find(r => r.offerId === (data._id || data.id));
  ...
  const isSaved = savedOffers.includes(offer._id || offer.id);

  const handleRedeem = () => {
    const code = redeemOffer(offer._id || offer.id);
    ...
    <div className="bg-background min-h-screen pb-20">
      ...
      <div className="relative h-[40vh] md:h-[50vh] w-full max-w-7xl mx-auto md:mt-6 md:rounded-[2rem] overflow-hidden shadow-sm">
        ...
        <div className="absolute top-4 right-4 flex gap-3">
          ...
          <button 
            onClick={() => user ? toggleSaveOffer(offer._id || offer.id) : alert('Login required')}
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-10 text-card">
          ...
          <div className="flex items-center gap-2 text-card/90 font-medium text-lg">
            {business.logo && <img src={business.logo} alt={business.name} className="w-6 h-6 rounded-full border border-card/50" />}
